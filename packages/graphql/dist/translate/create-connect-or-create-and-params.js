...
export function createConnectOrCreateAndParams({ input, varName, parentVar, relationField, refNode, node, context, withVars, callbackBucket, }) {
    asArray(input).forEach((connectOrCreateItem) => {
        ...
        if (conflictingProperties.length > 0) {
            throw new Neo4jGraphQLError(`Conflicting modification of ${conflictingProperties.map((n) => `[[${n}]]`).join(", ")} on type ${refNode.name}`);
        }
    });
    // todo: add create
    ...
    const wrappedQueries = statements.map((statement) => {
        ...
        const callStatement = new Cypher.Call(Cypher.concat(statement, returnStatement)).importWith(...withVarsVariables);
...
function createConnectOrCreatePartialStatement({ input, baseName, parentVar, relationField, refNode, node, context, callbackBucket, withVars, }) {
    ...
    if (authorizationBeforePredicateReturn.predicate) {
        if (authorizationBeforePredicateReturn.preComputedSubqueries) {
            mergeQuery = Cypher.concat(mergeQuery, new Cypher.With("*"), authorizationBeforePredicateReturn.preComputedSubqueries);
        }
        mergeQuery = Cypher.concat(mergeQuery, new Cypher.With("*").where(authorizationBeforePredicateReturn.predicate));
    }
    ...
    if (authorizationAfterPredicateReturn.predicate) {
        if (authorizationAfterPredicateReturn.preComputedSubqueries &&
            ...
            mergeQuery = Cypher.concat(mergeQuery, new Cypher.With(new Cypher.NamedVariable("*")), authorizationAfterPredicateReturn.preComputedSubqueries, new Cypher.With(new Cypher.NamedVariable("*")).where(authorizationAfterPredicateReturn.predicate));
        }
        else {
