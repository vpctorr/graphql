...
import { createAuthorizationAfterAndParams, createAuthorizationAfterAndParamsField, } from "./authorization/compatibility/create-authorization-after-and-params";
import { createAuthorizationBeforeAndParams, createAuthorizationBeforeAndParamsField, } from "./authorization/compatibility/create-authorization-before-and-params";
...
export default function createUpdateAndParams({ updateInput, varName, node, parentVar, chainStr, withVars, context, callbackBucket, parameterPrefix, includeRelationshipValidation, }) {
    ...
    function reducer(res, [key, value]) {
            ...
            refNodes.forEach((refNode, idx) => {
                ...
                updates.forEach((update, index) => {
                    ...
                    if (update.update) {
                        ...
                        if (update.where) {
                            try {
                                const { cypher: whereClause, subquery: preComputedSubqueries, params: whereParams, } = createConnectionWhereAndParams({
                        ...
                        subquery.push(`WITH ${withVars.join(", ")}`, "CALL {", indentBlock(innerUpdate.join("\n")), "}");
                        if (context.subscriptionsEnabled) {
                    ...
                    if (update.create) {
                        ...
                        creates.forEach((create, i) => {
                            ...
                            subquery.push(...getAuthorizationStatements(authorizationPredicates, authorizationSubqueries));
                            if (context.subscriptionsEnabled) {
        ...
        node.primitiveFields.forEach((field) => addCallbackAndSetParam(field, varName, updateInput, callbackBucket, res.strs, "UPDATE"));
        ...
        if (settableField) {
            ...
            if (pointField) {
                ...
                const mathDescriptor = mathDescriptorBuilder(value, node, mathMatch);
    ...
    if (meta.preArrayMethodValidationStrs.length) {
        ...
        preUpdatePredicates.push(`apoc.util.validatePredicate(${nullChecks.join(" OR ")}, "${pluralize("Property", propertyNames.length)} ${propertyNames.map(() => "%s").join(", ")} cannot be NULL", [${wrapStringInApostrophes(propertyNames).join(", ")}])`);
    }
