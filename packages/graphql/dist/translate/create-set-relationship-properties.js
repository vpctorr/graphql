...
function createSetRelationshipProperties({ properties, varName, withVars, relationship, relationshipAdapter, operation, callbackBucket, parameterPrefix, }) {
...
*/
function createSetRelationshipPropertiesForProperties({ properties, varName, withVars, relationship, operation, callbackBucket, parameterPrefix, }) {
    const strs = [];
    ...
    relationship.primitiveFields.forEach((field) => addCallbackAndSetParam(field, varName, properties, callbackBucket, strs, operation));
    Object.entries(properties).forEach(([key, value], _idx, propertiesEntries) => {
        ...
        if (hasMatched) {
            const mathDescriptor = mathDescriptorBuilder(value, relationship, mathMatch);
