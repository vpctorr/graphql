...
export function createAuthorizationBeforePredicate({ context, nodes, operations, }) {
    const predicates = [];
    let subqueries;
...
export function createAuthorizationBeforePredicateField({ context, nodes, operations, }) {
    const predicates = [];
    let subqueries;
    for (const nodeEntry of nodes) {
        const node = nodeEntry.node;
