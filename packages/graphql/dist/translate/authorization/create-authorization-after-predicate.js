...
export function createAuthorizationAfterPredicate({ context, nodes, operations, }) {
    const predicates = [];
    let subqueries;
...
export function createAuthorizationAfterPredicateField({ context, nodes, operations, conditionForEvaluation, }) {
    const predicates = [];
    let subqueries;
    for (const nodeEntry of nodes) {
        const node = nodeEntry.node;
