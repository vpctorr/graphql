    ...
    createAggregationOperation(entityOrRel, resolveTree, context) {
        ...
        if (entityOrRel instanceof RelationshipAdapter) {
            if (isConcreteEntity(entity)) {
                ...
                const nodeRawFields = {
                    ...parsedProjectionFields.node?.fieldsByTypeName[entityOrRel.operations.getAggregationFieldTypename("node")],
                };
            ...
            if (isConcreteEntity(entity)) {
                ...
                const fields = this.queryASTFactory.fieldFactory.createAggregationFields(entity, parsedProjectionFields.fields);
                operation.setFields(fields);
                const whereArgs = this.queryASTFactory.operationsFactory.getWhereArgs(resolveTree);
                const authFilters = this.queryASTFactory.authorizationFactory.getAuthFilters({
                    entity,
                    operations: ["AGGREGATE"],
                    attributes: this.queryASTFactory.operationsFactory.getSelectedAttributes(entity, parsedProjectionFields.fields),
                    context,
                });
