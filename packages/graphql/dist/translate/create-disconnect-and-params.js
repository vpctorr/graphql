...
function createDisconnectAndParams({ withVars, value, varName, relationField, parentVar, refNodes, context, labelOverride, parentNode, parameterPrefix, isFirstLevel = true, }) {
    ...
    function createSubqueryContents(relatedNode, disconnect, index) {
        ...
        if (disconnect.where) {
            try {
                const { cypher: whereCypher, subquery: preComputedSubqueries, params: whereParams, } = createConnectionWhereAndParams({
        ...
        subquery.push(`}`);
        if (context.subscriptionsEnabled) {
            subquery.push(`WITH ${filterMetaVariable(withVars).join(", ")}, ${variableName}, meta + update_meta as meta`);
        }
    ...
    function reducer(res, disconnect, index) {
        ...
        if (relationField.interface) {
            ...
            if (subqueries.length > 0) {
                if (context.subscriptionsEnabled) {
                    const withStatement = `WITH ${filterMetaVariable(withVars).join(", ")}, disconnect_meta + meta AS meta`;
