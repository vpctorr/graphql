...
export async function assertIndexesAndConstraints({ driver, sessionConfig, schemaModel, options, }) {
...
async function createIndexesAndConstraints({ schemaModel, session, }) {
                    ...
                    index.fields.forEach((field) => {
                        ...
                        if (!propertyIsInIndex) {
                            const aliasError = attribute.databaseName !== attribute.name
                                ...
                                : "";
                            indexErrors.push(`@fulltext index '${indexName}' on Node '${entity.name}' already exists, but is missing field '${field}'${aliasError}`);
                        }
                    });
                }
            });
...
async function getMissingConstraints({ schemaModel, session, }) {
    ...
    for (const entity of schemaModel.concreteEntities) {
        ...
        for (const uniqueField of entityAdapter.uniqueFields) {
            ...
            if (anyLabelHasConstraint === false) {
                ...
                const constraintName = uniqueField.annotations.unique.constraintName || `${entity.name}_${uniqueField.databaseName}`;
