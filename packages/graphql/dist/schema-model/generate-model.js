...
import { declareRelationshipDirective, nodeDirective, privateDirective, relationshipDirective, } from "../graphql/directives";
...
function hydrateInterfacesToTypeNamesMap(definitionCollection) {
    return definitionCollection.nodes.forEach((node) => {
        ...
        node.interfaces?.forEach((i) => {
            ...
            if (!concreteEntities) {
                throw new Neo4jGraphQLSchemaValidationError(`Could not find composite entity with name ${interfaceTypeName}`);
            }
...
function hydrateRelationshipDeclarations(definition, schema, definitionCollection) {
    ...
    for (const fieldDefinition of definition.fields) {
        ...
        if (relationshipField) {
            ...
            const allImplementationsPropertiesTypeNames = filterTruthy(relationshipField.relationshipImplementations.map((impl) => impl.propertiesTypeName));
...
function generateRelationshipField(field, schema, source, definitionCollection, firstDeclaredInTypeName, originalTarget) {
    ...
    const { type, direction, properties, queryDirection, nestedOperations, aggregate } = parseArguments(relationshipDirective, relationshipUsage);
    let attributes = [];
...
function generateRelationshipDeclaration(field, schema, source, definitionCollection, firstDeclaredInTypeName) {
    ...
    const { nestedOperations, aggregate } = parseArguments(declareRelationshipDirective, declareRelationshipUsage);
    const annotations = parseAnnotations(field.directives || []);
...
function generateConcreteEntity(definition, definitionCollection) {
    ...
    const schemaDirectives = definitionCollection.schemaExtension?.directives?.filter((x) => isInArray(SCHEMA_CONFIGURATION_OBJECT_DIRECTIVES, x.name.value));
...
function getLabels(entityDefinition) {
    ...
    if (nodeDirectiveUsage) {
        const nodeArguments = parseArguments(nodeDirective, nodeDirectiveUsage);
