...
export function parseLimitDirective({ directive, definition, }) {
...
function parseArgumentToInt(argument) {
    if (argument) {
        const parsed = parseValueNode(argument.value);
...
function validateLimitArguments(arg, typeName) {
    ...
    if (defaultLimit !== undefined && defaultLimit <= 0) {
        return new Neo4jGraphQLError(`${typeName} @limit(default: ${defaultLimit}) invalid value: '${defaultLimit}', it should be a number greater than 0`);
    }
    if (maxLimit !== undefined && maxLimit <= 0) {
        return new Neo4jGraphQLError(`${typeName} @limit(max: ${maxLimit}) invalid value: '${maxLimit}', it should be a number greater than 0`);
    }
    if (maxLimit && defaultLimit) {
        if (maxLimit < defaultLimit) {
            return new Neo4jGraphQLError(`${typeName} @limit(max: ${maxLimit}, default: ${defaultLimit}) invalid default value, 'default' must be smaller than 'max'`);
        }
    }
    return undefined;
}
