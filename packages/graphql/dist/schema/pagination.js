...
export function connectionFieldResolver({ connectionFieldName, source, args, info, }) {
 ...
 */
export function createConnectionWithEdgeProperties({ selectionSet, source, args = {}, totalCount, }) {
    ...
    const edgesField = selections.find((x) => x.kind === Kind.FIELD && x.name.value === "edges");
    ...
    const pageInfoField = selections.find((x) => x.kind === Kind.FIELD && x.name.value === "pageInfo");
