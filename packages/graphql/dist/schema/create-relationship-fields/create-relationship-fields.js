...
import { augmentObjectOrInterfaceTypeWithConnectionField, augmentObjectOrInterfaceTypeWithRelationshipField, } from "../generation/augment-object-or-interface";
...
import { augmentCreateInputTypeWithRelationshipsInput, withCreateInputType, withFieldInputType, } from "../generation/create-input";
...
export function createRelationshipFields({ entityAdapter, schemaComposer, 
...
composeNode, subgraph, userDefinedFieldDirectives, seenRelationshipPropertiesTypes, userDefinedDirectivesForNode, userDefinedFieldDirectivesForNode, features, }) {
    const relationships = entityAdapter instanceof ConcreteEntityAdapter
    ...
    relationships.forEach((relationshipAdapter) => {
        ...
        if (relationshipAdapter instanceof RelationshipAdapter) {
            if (relationshipAdapter.propertiesTypeName) {
                if (seenRelationshipPropertiesTypes.has(relationshipAdapter.propertiesTypeName)) {
                    ...
                    propertiesObjectType.setDescription(getRelationshipPropertiesTypeDescription({ relationshipAdapter, propertiesObjectType }));
                }
                else {
        ...
        const deprecatedDirectives = graphqlDirectivesToCompose((userDefinedDirectivesOnField || []).filter((directive) => directive.name.value === DEPRECATED));
...
function createRelationshipFieldsForTarget({ relationshipAdapter, composer, composeNode, userDefinedFieldDirectives, deprecatedDirectives, userDefinedDirectivesOnTargetFields, subgraph, // only for concrete targets
    ...
    composeNode.addFields(augmentObjectOrInterfaceTypeWithRelationshipField(relationshipAdapter, userDefinedFieldDirectives, subgraph));
    composeNode.addFields(augmentObjectOrInterfaceTypeWithConnectionField(relationshipAdapter, userDefinedFieldDirectives, composer));
