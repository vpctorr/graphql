...
export function ValidJwtDirectives(context) {
    const jwtTypes = new Map();
    const typeMap = new Map();
    const defs = context.getDocument().definitions;
    const objectsAndInterfaces = defs.filter((d) => d.kind === Kind.OBJECT_TYPE_DEFINITION ||
...
function assertJwtDirective(objectType, jwtTypes) {
    ...
    for (const typeName of jwtTypes.keys()) {
        if (!typeNamesForCurrent.includes(typeName)) {
            throw new DocumentValidationError(`Invalid directive usage: Directive @jwt can only be used once in the Type Definitions.`, []);
        }
    }
    ...
    for (const typeName of typeNamesForCurrent) {
        ...
        for (const t of types) {
            ...
            if (mergedDirectivesForCurrent.length > 1) {
                throw new DocumentValidationError(`Invalid directive usage: Directive @jwt cannot be used in combination with other directives.`, []);
            }
            const incompatibleFieldsStatus = t.fields?.some((field) => !GRAPHQL_BUILTIN_SCALAR_TYPES.includes(getInnerTypeName(field.type)));
            if (incompatibleFieldsStatus === true) {
                throw new DocumentValidationError(`Invalid directive usage: Fields of a @jwt type can only be Scalars or Lists of Scalars.`, []);
            }
        }
    }
}
function assertJwtClaimDirective(jwtTypes, node) {
    ...
    throw new DocumentValidationError(`Invalid directive usage: Directive @jwtClaim can only be used in \\"@jwt\\" types.`, []);
}
