...
export default function graphqlFormatter(neo4jStruct, readonly = false) {
...
function hydrateWithRelationships(nodes, rels) {
    Object.entries(rels).forEach(([relType, rel]) => {
        let relInterfaceName;
        if (rel.properties.length) {
            relInterfaceName = uniqueString(generateGraphQLSafeName(generateRelationshipPropsName(relType)), Object.values(nodes).map((n) => n.typeName));
        ...
        rel.paths.forEach((path) => {
            const { fromField, toField } = createRelationshipFields(nodes[path.fromTypeId].typeName, nodes[path.toTypeId].typeName, relType, relInterfaceName);
