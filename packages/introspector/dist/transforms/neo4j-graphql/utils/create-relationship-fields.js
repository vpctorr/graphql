...
export default function createRelationshipFields(fromTypeName, toTypeName, relType, propertiesTypeName) {
    const fromField = new NodeField(generateRelationshipFieldName(relType, fromTypeName, toTypeName, "OUT"), `[${toTypeName}!]!`);
    const fromDirective = new RelationshipDirective(relType, "OUT", propertiesTypeName);
    fromField.addDirective(fromDirective);
    const toField = new NodeField(generateRelationshipFieldName(relType, fromTypeName, toTypeName, "IN"), `[${fromTypeName}!]!`);
